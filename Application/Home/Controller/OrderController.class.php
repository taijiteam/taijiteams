...
class OrderController extends AdminController {
   ...
   public function protocol(){
     ...
     if ( IS_POST ) {
       ...
       if ( $response_update['status']=='0' ) {
         $re['code'] = '1';
         $re['message'] = '';
         $re['url'] = 'http://'.$_SERVER['HTTP_HOST'].'/merber_all_show/index.php/home/order/groupOrder';
       ...
       $memberInfo = $acitive->memberInfo($cardId,$password='',$isGetExtValue=true);
       $groupName = $memberInfo['data']['0']['MemberGroupName'];         //成员卡级别名称
       $protocol = $memberInfo['extValue']['0']['是否签署天朝上品微代理协议'];
       if ( $groupName=='待审核'||$groupName=='亦享成员') {
         $re['code'] = '1';
         $re['message'] = '请升级成员卡';
  ...
  public function groupOrder(){
    ...
    if ( $wechatOpenid['status']!='0' ) {
      ...
      if ( $groupName=='亦享成员'||$groupName=='待审核' ) {
        $re['code'] = '-1';
        $re['message'] = '您不是代理人无权访问该页面，请联系客服！';
        $re['url'] = 'http://www.qudaoplus.cn/index.php?m=content&c=index&a=lists&catid=25';
        $json = json_encode($re,JSON_UNESCAPED_UNICODE);
        $this->assign('json',$json);
      } elseif ( $protocol!='是' ) {
        $re['code'] = '-1';
        $re['message'] = '您未签署微代理协议';
        $re['url'] = 'http://'.$_SERVER['HTTP_HOST'].'/merber_all_show/index.php/home/order/protocol';
        $json = json_encode($re,JSON_UNESCAPED_UNICODE);
        $this->assign('json',$json);
      } else {
