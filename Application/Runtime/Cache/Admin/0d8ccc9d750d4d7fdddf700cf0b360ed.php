...
<div class="x-body">
  ...
  <xblock>
    ...
    <button class="layui-btn" onclick="x_admin_show('添加用户','<?php echo U('Admin/adminAdd');?>',700,600)"><i class="layui-icon"></i>添加</button>
  ...
  <table class="layui-table x-admin">
    ...
    <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr>
        ...
        <td><?php echo ($vo["a_note"]); ?></td>
        <td><?php echo ($vo["a_permissions"]); ?></td>
        <td><?php echo ($vo["a_time"]); ?></td>
        <td class="td-status">
          <?php if($vo["a_statat"] == 0): ?><span class="layui-btn layui-btn-normal layui-btn-mini">激活</span>
            <?php else: ?>
        ...
        <td class="td-manage">
          <a onclick="member_stop(this,'<?php echo ($vo["m_id"]); ?>')" id="<?php echo ($vo["id"]); ?>"  href="javascript:;"  title="<?php echo ($vo['a_statat']==0?'激活':'隐藏'); ?>">
            <i class="layui-icon" ><?php echo ($vo['m_starte']==0?'&#xe601;':'&#xe62f;'); ?></i>
          </a>
          <a title="编辑"  onclick="x_admin_show('编辑','<?php echo U('Admin/adminEdit');?>?id=<?php echo ($vo["m_id"]); ?>',600,400)" href="javascript:;">
            <i class="layui-icon">&#xe642;</i>
          </a>
          <a title="修改密码"  onclick="x_admin_show('修改密码','<?php echo U('Admin/adminPassword');?>?id=<?php echo ($vo["a_id"]); ?>',600,400)" href="javascript:;">
            <i class="layui-icon">&#xe631;</i>
          </a>
  ...
  function member_stop(obj,id){
    ...
    if ($(obj).attr('title')=='隐藏') {
      layer.confirm('确认要激活吗？',function(index) {
        $(obj).attr('title', '激活');
        $(obj).find('i').html('&#xe601;');
        $(obj).parents("tr").find(".td-status").find('span').removeClass('layui-btn-disabled').html('激活');
        layer.msg('已展示!', {icon: 1, time: 1000});
      });
    }else if($(obj).attr('title')=='激活'){
