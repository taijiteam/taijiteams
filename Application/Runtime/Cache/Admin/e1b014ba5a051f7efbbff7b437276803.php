...
<script>
    layui.use('table', function(){
        ...
        table.render({
            elem: '#demo'
            ,height: 800
            ,url: '/Admin/Order/check_ActivityList?source=<?php echo ($source); ?>'//数据接口
            ,title: '活动审核表'
            ,page: true //开启分页
            ,totalRow: true //开启合计行
            ,cols: [[ //表头
                ...
                ,{field: 'o_cate', title: '商品名称', width:150}
                ,{field: 'o_num', title: '数量', width:80, sort: true,totalRow: true}
                ,{field: 'o_name', title: '付款人', width:80}
                ,{field: 'o_phone', title: '手机号', width:177}
                ,{field: 'o_price', title: '支付价格', width: 100, sort: true,totalRow: true}
        ...
        table.on('tool(test)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'del'){
                layer.confirm('确定退回审核吗？', function(index){
                    ...
                    $.ajax({
                        type : "post",
                        url  : "/Admin/Order/activityReject",
                        data :{
                            id : data.order_id,
                        },
                        dataType : "json",
