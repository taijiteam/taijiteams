...
<div class="x-body">
    ...
    <table class="layui-table x-admin">
        ...
        <?php if(is_array($orderList)): $k = 0; $__LIST__ = $orderList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr>
                ...
                <td id="goods_num"><?php echo ($vo["goods_num"]); ?></td>
                <td id="goods_price"><?php echo ($vo["goods_price"]); ?></td>
                <td id="member_payment"><?php echo ($vo["member_payment"]); ?></td>
                <?php if($vo["order_status"] == 2): ?><td style="background-color: #FFA54F">待发货</td>
                    ...
                    <?php elseif($vo["order_status"] == 1): ?><td>待支付</td>
                    <?php else: ?><td></td><?php endif; ?>
                <!--<td style="color: #1aad19">
                    <?php if($vo["pocket_type"] == 1): ?>尊享积分
                        ...
                        无<?php endif; ?>
                    <br/>
                    <?php if($vo["pocket_value"] != 0 ): ?>-<?php echo ($vo["pocket_value"]); ?>
                        <?php else: endif; ?>
                </td>
                <?php if($vo["pocket_frm"] == 'bonus'): ?><td style="color: orange;">+<?php echo ($vo["pocket_value"]); ?></td>
                    <?php else: ?>
                    <td></td><?php endif; ?>-->

                <td style="color: #1aad19">微信支付</td>
                <?php if($vo["order_status"] == 13): ?><td style="background-color: 	#C3C3C3">驳回审核</td>
                    <?php elseif($vo["order_status"] == 4): ?><td style="background-color: rgba(4,174,115,0.42)">交易完成</td>
                    <?php elseif($vo["order_status"] == 6): ?><td style="background-color: rgba(4,174,115,0.42)">交易完成</td>
                    <?php elseif($vo["order_status"] == 9): ?><td>已取消</td>
                    <?php elseif($vo["order_status"] == 12): ?><td style="background-color: rgba(174,24,34,0.42)">已审核</td>
                    <?php else: ?><td>审核中</td><?php endif; ?>
                <td title="<?php echo ($vo["member_remark"]); ?>"><?php echo (subtext($vo["member_remark"],10)); ?></td>
                ...
                <td class="td-manage" style="font-size: 14px;">
                    ...
                    <a title="查看" href="/Admin/Operation/shopDetail?order_sn=<?php echo ($vo["order_sn"]); ?>"> 查看 </a>
                          <span>&nbsp;&nbsp;&nbsp;  |  &nbsp;&nbsp;&nbsp;</span>
...
<script>
    $("#btn").click(function () {
        ...
        $.ajax({
            url : "/Admin/Order/search?source=商城",
            type: "post",
            data:{
                //pay         : pay,
                audit       : audit,
