    ...
    <div class="x-body">
      ...
      <table class="layui-table x-admin">
        ...
        <?php if(is_array($consumption)): $k = 0; $__LIST__ = $consumption;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr>
            ...
            <?php if($vo["pocket_act"] == '0'): if($vo["pocket_type"] == '1'): ?><td>尊享积分<?php echo ($vo["pocket_value"]); ?></td>
            ...
            <?php if($vo["pocket_act"] == '1'): if($vo["pocket_type"] == '1'): ?><td>尊享积分<?php echo ($vo["pocket_value"]); ?></td>
                  ...
                  <?php else: ?>
                  <td></td><?php endif; ?>
            <td><?php echo ($vo["frm_data"]); ?></td>
            <?php if($vo["pocket_frm"] == 'admin'): ?><td>后台修改</td>
                <?php elseif($vo["pocket_frm"] == 'order'): ?>
                <td>购买消费</td>
                <?php elseif($vo["pocket_type"] == 'bonus'): ?>
                <td>下单返利</td>
                <?php elseif($vo["pocket_type"] == 'dsign'): ?>
                <td>每日签到</td>
                <?php elseif($vo["pocket_type"] == 'order_cancel'): ?>
                <td>订单取消</td>
                <?php else: ?>
                <td></td><?php endif; ?>
            <!--<td style="color: red"><?php echo ($memberInfo['m_integrals']); ?></td>-->
            <!--<td class="td-statuss">
                <span class="layui-btn layui-btn-normal layui-btn-mini" onclick="x_admin_show('详情','<?php echo U('Sort/sortDefatu');?>?id=<?php echo ($vo["a_aid"]); ?>',500,420)"  href="javascript:;">详情</span>
            </td>-->
