...
<head>
	<meta charset="utf-8">
	<title>渠道PLUS微管家商户平台V1.0</title>
<link rel="stylesheet" type="text/css" href="/Public/AdminShops/css/ertao.css">
</head>
<body class="login">
	<header>渠道PLUS微管家商户平台V1.0</header>
	<div class="cont">
		<p><img src="/Public/AdminShops/images/icon/logo.png"></p>
		<form  method="post" class="layui-form" method="post">
	...
	<footer>渠道PLUS © 2019-2022</footer>
<!-- 请在下方写相关业务代码 -->
...
<script type="text/javascript">
	$(function  () {
		layui.use('form', function(){
			var form = layui.form;
			layer.msg('欢迎使用渠道PLUS微管家商户平台V1.0！', function(){
				//关闭后的操作
			});
			//监听提交
			form.on('submit(login)', function(data){
				var username = $('input[name="username"]').val();	//出行时间
				var password = $('input[name="password"]').val();	//请输入到店时间
				if(username == ''){
					$.Huimodalalert('请输入账号！',1500);
				}else if(password == ''){
					$.Huimodalalert('请输入密码！',1500);
				}else{
					$.ajax({
						type:"post",
						url:"<?php echo U('Index/login');?>",//根据自己项目的需要写请求地址
						data:{
							'username':username,
							'password':password,
						},
						...
						success:function(data){
							...
							if (data == 1){
								layer.confirm('恭喜您登录成功！',function(index){location.href="<?php echo U('Index/index');?>"});
							}else{
								layer.msg('哎呦，账号或密码输入错误哦！',function(){});
							}
						},
						error:function(data) {
