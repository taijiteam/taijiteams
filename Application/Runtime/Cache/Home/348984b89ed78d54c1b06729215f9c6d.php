...
<body>
	...
	<div class="hospital">
		<h3><img src="/Public/Doctor/images/yuan1.png">名院展示</h3>
		<ul class="list">
			<?php if(is_array($h_list)): $i = 0; $__LIST__ = $h_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$h_list): $mod = ($i % 2 );++$i;?><a href="<?php echo U('Doctor/hospital_content');?>?h_id=<?php echo ($h_list["h_id"]); ?>">
				<li>
					...
					<div>
						<h4><?php echo ($h_list["h_hospital"]); ?></h4>
						<p>简介：<?php echo (mb_substr(strip_tags(htmlspecialchars_decode($h_list["h_introduction"])),0,30,'utf-8')); ?>...</p>
					</div>
					<object><a href="<?php echo U('Doctor/hospital_content');?>?h_id=<?php echo ($h_list["h_id"]); ?>">详情&gt;&gt;</a></object>
				</li>
